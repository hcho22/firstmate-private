...
finish_concurrent_recovery() {  # <id> <home> <status> <stdout> <stderr>
  ...
  spawn_task "$id" "$home" "$PROJECT_DIR" > "$out" 2> "$err" \
    || fail "recovery $id retry failed after the concurrent recovery completed: $(cat "$err")"
}
...
# Two homes recovering concurrently make bounded attempts on the named session
# lock and each replace only their own exact husk, retrying a contended peer
# after the lock holder completes.
