...
# Install cleanup before lock acquisition begins so an interruption inside the
# acquisition cannot leave a lock that already names this watcher behind.
WATCHER_PID=${BASHPID:-$$}
WATCHER_STARTED=0
...
watcher_cleanup() {
  ...
  if [ "$owns_lock" -eq 1 ] || [ "$WATCHER_STARTED" -eq 1 ]; then
    fm_active_check_stop || cleanup_status=1
    fm_check_output_cleanup
    fm_custom_check_snapshot_cleanup
  fi
  if [ "$owns_lock" -eq 1 ]; then
    if [ "$WATCHER_RECOVERY_PENDING" -eq 1 ] \
