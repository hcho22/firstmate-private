...
The retained timing baseline comes from the `fm-test-timing-portable-serial-*` artifacts of three green CI runs on 2026-09-01, [33558082172](https://github.com/kunchenguid/firstmate/actions/runs/33558082172), [33523597838](https://github.com/kunchenguid/firstmate/actions/runs/33523597838), and [33463326167](https://github.com/kunchenguid/firstmate/actions/runs/33463326167).
...
Use the runner's `--list-lanes` and `--list --lane <name>` inspection interfaces for current membership, and its JSON timing artifacts for measured durations; added tests change the partition.
...
Refresh the hints by downloading the per-shard timing artifacts from several green CI runs and replacing the `portable_serial_weight_hints` table in `bin/fm-test-run.sh` with the slowest measured `duration_ms` per `path`:
...
| portable serial 1-5 | job `timeout-minutes: 30` | The hang tripwire stays above the roughly 20-minute slow-runner spread observed in CI; current shard durations come from the runner's timing artifacts. |
